...
// Helper function to generate all days from periods structure
const generateAllDays = (periods: any[]) => {
  const allDays: any[] = [];
  periods.forEach(period => {
    period.cycles.forEach((cycle: any) => {
...
// Generate initial data for periods first
...
const allDays = generateAllDays(initialPeriods);

// Generate initial time relative cells (24 hours for all days)
const initialTimeRelativeCells = allDays.map((day, index) => ({
  id: `time-relative-${day.id}`,
  dayId: day.id,
  value: 24
}));

// Generate initial time window cells (varying hours)
const initialTimeWindowCells = allDays.map((day, index) => ({
  id: `time-window-${day.id}`,
  dayId: day.id,
  value: index % 3 === 0 ? 24 : index % 3 === 1 ? 2 : 4
}));

// Generate initial time of day cells (alternating morning/afternoon)
const initialTimeOfDayCells = allDays.map((day, index) => ({
